...
class Network(nn.Module):
    def __init__(self, config,wavelet_name):
        ...
        self.stage1 = self._make_stage(n_channels[0],
                                       ...
                                       block,
                                       stride=1)
        self.stage21 = self._make_stage(n_channels[0],
                                       n_channels[1],
                                       2,
                                       block,
                                       stride=2)
        self.stage22 = self._make_stage(n_channels[1],
                                       n_channels[1],
                                       2,
                                       block,
                                       stride=1)
        self.stage31 = self._make_stage(n_channels[1],
                                       n_channels[2],
                                       3,
                                       block,
                                       stride=2)
        self.stage32 = self._make_stage(n_channels[2],
                                       n_channels[2],
                                       3,
                                       block,
                                       stride=1)
        ...
        self.wavelet_block21 = Block_lite_lite(in_channels = 64, out_channels = 512, is_supplement = True, wavelet_name = wavelet_name)
        self.wavelet_block22 = Block_lite_lite(in_channels = 512, out_channels = 512, is_supplement = True, wavelet_name = wavelet_name)
        self.wavelet_block31 = Block_lite_lite(in_channels = 512, out_channels = 1024, is_supplement = True, wavelet_name = wavelet_name)
        self.wavelet_block32 = Block_lite_lite(in_channels = 1024, out_channels = 1024, is_supplement = True, wavelet_name = wavelet_name)
        self.wavelet_block4 = Block_lite_lite(in_channels = 1024, out_channels = 2048, is_supplement = True, wavelet_name = wavelet_name)
    ...
    def _forward_conv(self, x):
        ...
        x = self.stage21(x)
        x, Lnext = self.wavelet_block21(Llast = Lnext, original = Lx32, x = x, is_downsample = False)

        x = self.stage22(x)
        x, Lnext = self.wavelet_block22(Llast = Lnext, original = Lx32, x = x, is_downsample = True)
        
    
        x = self.stage31(x)
        x, Lnext = self.wavelet_block31(Llast = Lnext, original = Lx16, x = x, is_downsample = False)

 
        x = self.stage32(x)        
        x, Lnext = self.wavelet_block32(Llast = Lnext, original = Lx16, x = x, is_downsample = True)
